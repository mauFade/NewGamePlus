...
class UserInstant {
  ...
  async update(request: Request, response: Response) {
    try {
      ...
      // Declara um auxiliar para atualizar o user
      const auxUser: IUser[] = await connection("Users").where({ email: email }).select("*").update(update);

      return response.status(200).send(new CSuccess(true, `User updated successfully. New data: ${auxUser}`));
      // Caso algo tenha dado errado
    } catch (error) {
      // Retorna erro
