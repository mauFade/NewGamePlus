...
class PostInstant {
  ...
  async read(request: Request, response: Response) {
    try {
      ...
      if (!id) {
        return response.status(403).send(new CError("Erro at method read.", "You must be logged in to view this."));
      }
      // Busca usuários e posts do banco
      ...
      for (const index of posts) {
        for (const user of users) {
          posts[index]["user_id"] === users[user]["user_id"]
            ? posts[index]["name"] === users[user]["name"] && posts[index]["email"] === users[user]["email"]
            : posts[index]["name"] === "Unknown" && posts[index]["email"] === "Unknown";
        }
      }
