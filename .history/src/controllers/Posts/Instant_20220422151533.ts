...
class PostInstant {
  ...
  async read(request: Request, response: Response) {
    try {
      ...
      const output: ITimeline[] = await connection("Posts").join("Users", "Users.id", "=", "Posts.user_id").select("*");

      // Iteracção para não retornar dados não desejados
      const allowedKeys = ["post_id", "post_name", "post_description", "specific_game", "name", "email", "city"];
      Object.keys(output).forEach((key) => {
        if (!allowedKeys.includes(key)) delete output[0][key];
      });
